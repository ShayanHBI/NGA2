...
module tplowmach_class
   ...
   !> Two-phase low-mach solver object definition
   type :: tplowmach
      ...
      character(len=str_medium) :: name='UNNAMED_tplowmach'               !< Solver name (default=UNNAMED_tplowmach)
      ...
      ! Density fields
      real(WP), dimension(:,:,:), allocatable :: rho_l                    !< Density field for liquid
      real(WP), dimension(:,:,:), allocatable :: rho_g                    !< Density field for gas
      ...
      ! Viscosity fields
      real(WP), dimension(:,:,:), allocatable :: visc_l                   !< Viscosity field for liquid
      real(WP), dimension(:,:,:), allocatable :: visc_g                   !< Viscosity field for gas
   ...
   contains
      procedure :: print=>tplowmach_print                                 !< Output solver to the screen
   ...
   !> Declare Two-phase low-mach solver constructor
   interface tplowmach
      procedure constructor
   end interface tplowmach

contains


   !> Default constructor for two-phase low-mach flow solver
   function constructor(cfg,name) result(self)
      ...
      allocate(self%rho_l  (self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%rho_l  =0.0_WP
      allocate(self%rho_g  (self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%rho_g  =0.0_WP
   ...
   subroutine init_metrics(this)
      implicit none
      class(tplowmach), intent(inout) :: this
   ...
   subroutine add_bcond(this,name,type,locator,face,dir,canCorrect)
      ...
      select case (lowercase(face))
       ...
       case default; call die('[tplowmach add_bcond] Unknown bcond face - expecting x, y, or z')
      end select
      ...
      select case (dir) ! Outward-oriented
       ...
       case (-1); new_bc%dir=-1
       case ( 0); new_bc%dir= 0
       case default; call die('[tplowmach add_bcond] Unknown bcond dir - expecting -1, +1, or 0')
      end select
      ...
      select case (new_bc%type)
       ...
       case (slip)
       case default
         call die('[tplowmach apply_bcond] Unknown bcond type')
      end select
   ...
   subroutine get_bcond(this,name,my_bc)
      ...
      if (.not.associated(my_bc)) call die('[tplowmach get_bcond] Boundary condition was not found')
   ...
   subroutine prepare_advection_semilag(this,dt,vf)
      ...
      implicit none
      class(tplowmach), intent(inout) :: this   !< The two-phase flow solver
   ...
   subroutine prepare_advection_upwind(this,dt)
      ...
      real(WP) :: rhoeps,vel,rho_l_min,rho_g_min

      ! Parameter for switching to the upwind interpolation scheme (QO: Are the following three lines enough for rhoeps?)
      rho_l_min=minval(this%rho_l)
      rho_g_min=minval(this%rho_g)
      rhoeps=rhoeps_coeff*min(rho_l_min,rho_g_min)
   ...
   subroutine get_dmomdt(this,drhoUdt,drhoVdt,drhoWdt)
      ...
      integer :: i,j,k,ii,jj,kk
      real(WP) :: divu  !< Local velocity divergence for bulk viscosity correction
      ...
      do kk=this%cfg%kmin_,this%cfg%kmax_+1
         do jj=this%cfg%jmin_,this%cfg%jmax_+1
            do ii=this%cfg%imin_,this%cfg%imax_+1
               ! Fluxes on x-face
               i=ii-1; j=jj-1; k=kk-1
               ! Local velocity divergence at cell (i,j,k) for bulk viscosity correction
               divu=sum(this%grdu_x(:,i,j,k)*this%U(i:i+1,j,k))+ &
               &    sum(this%grdv_y(:,i,j,k)*this%V(i,j:j+1,k))+ &
               &    sum(this%grdw_z(:,i,j,k)*this%W(i,j,k:k+1))
               ...
               &              +this%visc   (i,j,k)*(sum(this%grdu_x(:,i,j,k)*this%U(i:i+1,j,k))+sum(this%grdu_x(:,i,j,k)*this%U(i:i+1,j,k))-(2.0_WP/3.0_WP)*divu)
               ! Fluxes on y-face
               i=ii; j=jj; k=kk
      ...
      do kk=this%cfg%kmin_,this%cfg%kmax_+1
         do jj=this%cfg%jmin_,this%cfg%jmax_+1
            do ii=this%cfg%imin_,this%cfg%imax_+1
               ...
               &              +this%visc   (i,j,k)*(sum(this%grdv_y(:,i,j,k)*this%V(i,j:j+1,k))+sum(this%grdv_y(:,i,j,k)*this%V(i,j:j+1,k))-(2.0_WP/3.0_WP)*divu)
               ! Fluxes on z-face
               i=ii; j=jj; k=kk
      ...
      do kk=this%cfg%kmin_,this%cfg%kmax_+1
         do jj=this%cfg%jmin_,this%cfg%jmax_+1
            do ii=this%cfg%imin_,this%cfg%imax_+1
               ...
               &              +this%visc   (i,j,k)*(sum(this%grdw_z(:,i,j,k)*this%W(i,j,k:k+1))+sum(this%grdw_z(:,i,j,k)*this%W(i,j,k:k+1))-(2.0_WP/3.0_WP)*divu)
            end do
         end do
      end do
   ...
   subroutine add_surface_tension_jump(this,dt,div,vf,contact_model)
      ...
      if (present(contact_model)) then
         select case (contact_model)
          ...
          case default
            call die('[tplowmach: add_surface_tension_jump] Unknown contact model!')
         end select
      end if
   ...
   subroutine get_strainrate(this,SR)
      ...
      if (size(SR,dim=1).ne.6) call die('[tplowmach get_strainrate] SR should be of size (1:6,imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)')
   ...
   subroutine get_ugradu(this,ugradu)
      ...
      if (size(ugradu,dim=1).ne.3) call die('[tplowmach get_ugradu] gradu should be of size (1:3,imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)')
   ...
   subroutine get_gradu(this,gradu)
      ...
      if (size(gradu,dim=1).ne.3.or.size(gradu,dim=2).ne.3) call die('[tplowmach get_gradu] gradu should be of size (1:3,1:3,imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)')
   ...
   subroutine get_vorticity(this,vort)
      ...
      if (size(vort,dim=1).ne.3) call die('[tplowmach get_vorticity] vort should be of size (1:3,imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)')
   ...
   subroutine get_cfl(this,dt,cflc,cfl)
      ...
      real(WP) :: max_nu,max_nu_l,max_nu_g
      ...
      if (this%sigma.gt.0.0_WP) then
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  my_CFLst=min(my_CFLst,sqrt((this%rho_l(i,j,k)+this%rho_g(i,j,k))*this%cfg%meshsize(i,j,k)**3.0_WP/(4.0_WP*Pi*this%sigma)))
               end do
            end do
         end do
      end if
      ...
      max_nu_l=maxval(this%visc_l/this%rho_l)
      max_nu_g=maxval(this%visc_g/this%rho_g)
      max_nu=max(max_nu_l,max_nu_g)
   ...
   subroutine shift_p(this,pressure)
      implicit none
      class(tplowmach), intent(in) :: this
   ...
   subroutine get_viscosity(this,vf,strat)
      ...
      real(WP) :: liq_vol_visc_l,gas_vol_visc_g
         ...
         do k=this%cfg%kmino_+1,this%cfg%kmaxo_
            do j=this%cfg%jmino_+1,this%cfg%jmaxo_
               do i=this%cfg%imino_+1,this%cfg%imaxo_
                  ...
                  tot_vol=gas_vol+liq_vol
                  liq_vol_visc_l=sum(vf%Lvol(0,0,:,i,j,k))/this%visc_l(i,j,k)+sum(vf%Lvol(1,0,:,i-1,j,k))/this%visc_l(i-1,j,k)+sum(vf%Lvol(0,1,:,i,j-1,k))/this%visc_l(i,j-1,k)+sum(vf%Lvol(1,1,:,i-1,j-1,k))/this%visc_l(i-1,j-1,k)
                  gas_vol_visc_g=sum(vf%Gvol(0,0,:,i,j,k))/this%visc_g(i,j,k)+sum(vf%Gvol(1,0,:,i-1,j,k))/this%visc_g(i-1,j,k)+sum(vf%Gvol(0,1,:,i,j-1,k))/this%visc_g(i,j-1,k)+sum(vf%Gvol(1,1,:,i-1,j-1,k))/this%visc_g(i-1,j-1,k)
                  this%visc_xy(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_xy(i,j,k)=tot_vol/(liq_vol_visc_l+gas_vol_visc_g+epsilon(1.0_WP))
                  ...
                  tot_vol=gas_vol+liq_vol
                  liq_vol_visc_l=sum(vf%Lvol(:,0,0,i,j,k))/this%visc_l(i,j,k)+sum(vf%Lvol(:,1,0,i,j-1,k))/this%visc_l(i,j-1,k)+sum(vf%Lvol(:,0,1,i,j,k-1))/this%visc_l(i,j,k-1)+sum(vf%Lvol(:,1,1,i,j-1,k-1))/this%visc_l(i,j-1,k-1)
                  gas_vol_visc_g=sum(vf%Gvol(:,0,0,i,j,k))/this%visc_g(i,j,k)+sum(vf%Gvol(:,1,0,i,j-1,k))/this%visc_g(i,j-1,k)+sum(vf%Gvol(:,0,1,i,j,k-1))/this%visc_g(i,j,k-1)+sum(vf%Gvol(:,1,1,i,j-1,k-1))/this%visc_g(i,j-1,k-1)
                  this%visc_yz(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_yz(i,j,k)=tot_vol/(liq_vol_visc_l+gas_vol_visc_g+epsilon(1.0_WP))
                  ...
                  tot_vol=gas_vol+liq_vol
                  liq_vol_visc_l=sum(vf%Lvol(0,:,0,i,j,k))/this%visc_l(i,j,k)+sum(vf%Lvol(0,:,1,i,j,k-1))/this%visc_l(i,j,k-1)+sum(vf%Lvol(1,:,0,i-1,j,k))/this%visc_l(i-1,j,k)+sum(vf%Lvol(1,:,1,i-1,j,k-1))/this%visc_l(i-1,j,k-1)
                  gas_vol_visc_g=sum(vf%Gvol(0,:,0,i,j,k))/this%visc_g(i,j,k)+sum(vf%Gvol(0,:,1,i,j,k-1))/this%visc_g(i,j,k-1)+sum(vf%Gvol(1,:,0,i-1,j,k))/this%visc_g(i-1,j,k)+sum(vf%Gvol(1,:,1,i-1,j,k-1))/this%visc_g(i-1,j,k-1)
                  this%visc_zx(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_zx(i,j,k)=tot_vol/(liq_vol_visc_l+gas_vol_visc_g+epsilon(1.0_WP))
               end do
            end do
         end do
         ...
         do k=this%cfg%kmino_+1,this%cfg%kmaxo_
            do j=this%cfg%jmino_+1,this%cfg%jmaxo_
               do i=this%cfg%imino_+1,this%cfg%imaxo_
                  ...
                  gas_vol_visc_g=sum(vf%Gvol(0,0,:,i,j,k))*this%visc_g(i,j,k)+sum(vf%Gvol(1,0,:,i-1,j,k))*this%visc_g(i-1,j,k)+sum(vf%Gvol(0,1,:,i,j-1,k))*this%visc_g(i,j-1,k)+sum(vf%Gvol(1,1,:,i-1,j-1,k))*this%visc_g(i-1,j-1,k)
                  this%visc_xy(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_xy(i,j,k)=(liq_vol_visc_l+gas_vol_visc_g)/tot_vol
                  ! VISC_yz at [xm,y,z] - direct sum in x, staggered sum in y/z
                  liq_vol=sum(vf%Lvol(:,0,0,i,j,k))+sum(vf%Lvol(:,1,0,i,j-1,k))+sum(vf%Lvol(:,0,1,i,j,k-1))+sum(vf%Lvol(:,1,1,i,j-1,k-1))
                  gas_vol=sum(vf%Gvol(:,0,0,i,j,k))+sum(vf%Gvol(:,1,0,i,j-1,k))+sum(vf%Gvol(:,0,1,i,j,k-1))+sum(vf%Gvol(:,1,1,i,j-1,k-1))
                  tot_vol=gas_vol+liq_vol
                  ...
                  gas_vol_visc_g=sum(vf%Gvol(:,0,0,i,j,k))*this%visc_g(i,j,k)+sum(vf%Gvol(:,1,0,i,j-1,k))*this%visc_g(i,j-1,k)+sum(vf%Gvol(:,0,1,i,j,k-1))*this%visc_g(i,j,k-1)+sum(vf%Gvol(:,1,1,i,j-1,k-1))*this%visc_g(i,j-1,k-1)
                  this%visc_yz(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_yz(i,j,k)=(liq_vol_visc_l+gas_vol_visc_g)/tot_vol
                  ! VISC_zx at [x,ym,z] - direct sum in y, staggered sum in z/x
                  liq_vol=sum(vf%Lvol(0,:,0,i,j,k))+sum(vf%Lvol(0,:,1,i,j,k-1))+sum(vf%Lvol(1,:,0,i-1,j,k))+sum(vf%Lvol(1,:,1,i-1,j,k-1))
                  gas_vol=sum(vf%Gvol(0,:,0,i,j,k))+sum(vf%Gvol(0,:,1,i,j,k-1))+sum(vf%Gvol(1,:,0,i-1,j,k))+sum(vf%Gvol(1,:,1,i-1,j,k-1))
                  tot_vol=gas_vol+liq_vol
                  ...
                  gas_vol_visc_g=sum(vf%Gvol(0,:,0,i,j,k))*this%visc_g(i,j,k)+sum(vf%Gvol(0,:,1,i,j,k-1))*this%visc_g(i,j,k-1)+sum(vf%Gvol(1,:,0,i-1,j,k))*this%visc_g(i-1,j,k)+sum(vf%Gvol(1,:,1,i-1,j,k-1))*this%visc_g(i-1,j,k-1)
                  this%visc_zx(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_zx(i,j,k)=(liq_vol_visc_l+gas_vol_visc_g)/tot_vol
               end do
            end do
         end do
       case default
         call die('[tplowmach get_viscosity] Unknown viscosity averaging strategy')
      end select
      ...
      do k=this%cfg%kmino_+1,this%cfg%kmaxo_
         do j=this%cfg%jmino_+1,this%cfg%jmaxo_
            do i=this%cfg%imino_+1,this%cfg%imaxo_
               ...
               tot_vol=gas_vol+liq_vol
               liq_mass=sum(vf%Lvol(0,:,:,i,j,k))*this%rho_l(i,j,k)+sum(vf%Lvol(1,:,:,i-1,j,k))*this%rho_l(i-1,j,k)
               gas_mass=sum(vf%Gvol(0,:,:,i,j,k))*this%rho_g(i,j,k)+sum(vf%Gvol(1,:,:,i-1,j,k))*this%rho_g(i-1,j,k)
               this%rho_Uold(i,j,k)=1.0_WP
               if (tot_vol.gt.0.0_WP) this%rho_Uold(i,j,k)=(liq_mass+gas_mass)/tot_vol
               ! V-cell
               liq_vol=sum(vf%Lvol(:,0,:,i,j,k))+sum(vf%Lvol(:,1,:,i,j-1,k))
               gas_vol=sum(vf%Gvol(:,0,:,i,j,k))+sum(vf%Gvol(:,1,:,i,j-1,k))
               tot_vol=gas_vol+liq_vol
               liq_mass=sum(vf%Lvol(:,0,:,i,j,k))*this%rho_l(i,j,k)+sum(vf%Lvol(:,1,:,i,j-1,k))*this%rho_l(i,j-1,k)
               gas_mass=sum(vf%Gvol(:,0,:,i,j,k))*this%rho_g(i,j,k)+sum(vf%Gvol(:,1,:,i,j-1,k))*this%rho_g(i,j-1,k)
               this%rho_Vold(i,j,k)=1.0_WP
               if (tot_vol.gt.0.0_WP) this%rho_Vold(i,j,k)=(liq_mass+gas_mass)/tot_vol
               ! W-cell
               liq_vol=sum(vf%Lvol(:,:,0,i,j,k))+sum(vf%Lvol(:,:,1,i,j,k-1))
               gas_vol=sum(vf%Gvol(:,:,0,i,j,k))+sum(vf%Gvol(:,:,1,i,j,k-1))
               tot_vol=gas_vol+liq_vol
               liq_mass=sum(vf%Lvol(:,:,0,i,j,k))*this%rho_l(i,j,k)+sum(vf%Lvol(:,:,1,i,j,k-1))*this%rho_l(i,j,k-1)
               gas_mass=sum(vf%Gvol(:,:,0,i,j,k))*this%rho_g(i,j,k)+sum(vf%Gvol(:,:,1,i,j,k-1))*this%rho_g(i,j,k-1)
               this%rho_Wold(i,j,k)=1.0_WP
               if (tot_vol.gt.0.0_WP) this%rho_Wold(i,j,k)=(liq_mass+gas_mass)/tot_vol
            end do
         end do
      end do
      ! Synchronize boundaries - not really needed...
   ...
   !> Print out info for Two-phase low-mach flow solver
   subroutine tplowmach_print(this)
      ...
      if (this%cfg%amRoot) then
         write(output_unit,'("Two-phase low-mach solver [",a,"] for config [",a,"]")') trim(this%name),trim(this%cfg%name)
   ...
   end subroutine tplowmach_print


end module tplowmach_class
