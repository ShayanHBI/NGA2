...
module amrmpcomp_class
   ...
   type, extends(amrvof) :: amrmpcomp
      ...
      ! Gas equation of state function pointers: PG=PG(rho,I,Yv), CG=CG(rho,P,Yv), TG=TG(rho,P,Yv)
      ...
      ! Conserved variables (1: VF*rhoL, 2: (1-VF)*rhoG, 3: VF*rhoL*IL, 4: (1-VF)*rhoG*IG, 5: rhoU, 6: rhoV, 7: rhoW, 8: (1-VF)*rhoG*Yv)
      type(amrdata) :: Q,Qold
      ...
      real(WP), dimension(8) :: Qint=0.0_WP,Qmin=0.0_WP,Qmax=0.0_WP
...
contains
   ...
   subroutine initialize(this,amr,name)
      ...
      ! Initialize conserved variables Q (8 components for multiphase)
      call this%Q%initialize   (amr,name='Q'   ,ncomp=8,ng=this%nover); this%Q%parent   =>this
      call this%Qold%initialize(amr,name='Qold',ncomp=8,ng=this%nover); this%Qold%parent=>this
   ...
   subroutine finalize(this)
      ...
      ! Physical properties
      call this%visc%finalize(); call this%beta%finalize(); call this%cond%finalize(); call this%diff%finalize()
      ! Nullify pointers
   ...
   subroutine get_primitive(this,Q)
      ...
      real(WP), dimension(:,:,:,:), contiguous, pointer :: pRHOL,pRHOG,pIL,pIG,pPL,pPG,pTL,pTG,pC,pYv
         ...
         do while (mfi%next())
            ...
            do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
               ...
               if (pQ(i,j,k,2).gt.0.0_WP) then
                  pYv(i,j,k,1)=pQ(i,j,k,8)/pQ(i,j,k,2)
               else
                  pYv(i,j,k,1)=0.0_WP
               end if
               ...
               if (pVF(i,j,k,1).le.VFhi.and.pQ(i,j,k,2).gt.0.0_WP.and.pQ(i,j,k,4).gt.0.0_WP) then
                  ...
                  pPG  (i,j,k,1)=this%getPG(pRHOG(i,j,k,1),pIG(i,j,k,1),pYv(i,j,k,1))
                  pTG  (i,j,k,1)=this%getTG(pRHOG(i,j,k,1),pPG(i,j,k,1),pYv(i,j,k,1))
                  CG            =this%getCG(pRHOG(i,j,k,1),pPG(i,j,k,1),pYv(i,j,k,1))
               else
   ...
   subroutine get_conserved(this)
      ...
      type(amrex_mfiter) :: mfi
      type(amrex_box) :: bx
      real(WP), dimension(:,:,:,:), contiguous, pointer :: pQ,pVF,pU,pV,pW,pRHOL,pRHOG,pIL,pIG,pYv
      do lvl=0,this%amr%clvl()
         call this%amr%mfiter_build(lvl,mfi)
         do while (mfi%next())
            ! Get pointers to data
            ...
            do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
               ...
               pQ(i,j,k,8)=pQ(i,j,k,2)*pYv(i,j,k,1)
            end do; end do; end do
         end do
         call this%amr%mfiter_destroy(mfi)
      end do
   ...
   subroutine get_dQdt(this,Q,dQdt,dt,time)
      ...
      define_fluxes: block
         integer :: lvl
         ! Face-centered conserved variable fluxes (8 components)
         do lvl=0,this%amr%clvl()
      ...
      finitevolume_fluxes: block
         ...
         real(WP), dimension(:,:,:,:), contiguous, pointer :: pPL,pPG,pTL,pTG,pIL,pIG,pVF,pYv
         real(WP), dimension(:,:,:,:), contiguous, pointer :: pVisc,pBeta,pCond,pDiff
         ...
         real(WP) :: w,div,vel,visc_f,beta_f,irho_f,face_apt_g,GVFm,GVFp
            ...
            do while (mfi%next())
               ...
               do k=fbx%lo(3),fbx%hi(3); do j=fbx%lo(2),fbx%hi(2); do i=fbx%lo(1),fbx%hi(1)
                  ...
                  if (.not.in_band) then
                     ...
                     ! WENO gas mass, energy, and vapor fluxes
                  ...
                  face_apt_g=0.0_WP
                  if (GVFm.gt.VFlo.and.GVFp.gt.VFlo) then
                     if (GVFm.ge.VFhi.or.GVFp.ge.VFhi) then
                        face_apt_g=1.0_WP
                     else
                        face_apt_g=0.5_WP*(GVFm+GVFp)
                     end if
                  end if
                  pFx(i,j,k,8)=pFx(i,j,k,8)+0.5_WP*sum(pDiff(i-1:i,j,k,1))*dxi*(pYv(i,j,k,1)-pYv(i-1,j,k,1))*face_apt_g
               end do; end do; end do
               ! Y-fluxes
               fbx=mfi%nodaltilebox(2)
               do k=fbx%lo(3),fbx%hi(3); do j=fbx%lo(2),fbx%hi(2); do i=fbx%lo(1),fbx%hi(1)
                  ...
                  pFy(i,j,k,8)=pFy(i,j,k,8)+0.5_WP*sum(pDiff(i,j-1:j,k,1))*dyi*(pYv(i,j,k,1)-pYv(i,j-1,k,1))*face_apt_g
               end do; end do; end do
               ! Z-fluxes
               fbx=mfi%nodaltilebox(3)
               do k=fbx%lo(3),fbx%hi(3); do j=fbx%lo(2),fbx%hi(2); do i=fbx%lo(1),fbx%hi(1)
                  ...
                  pFz(i,j,k,8)=pFz(i,j,k,8)+0.5_WP*sum(pDiff(i,j,k-1:k,1))*dzi*(pYv(i,j,k,1)-pYv(i,j,k-1,1))*face_apt_g
               end do; end do; end do
            end do
            call this%amr%mfiter_destroy(mfi)
         end do
         ...
         do lvl=0,this%amr%clvl()
            ...
            do while (mfi%next())
               ...
               do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
                  ...
                  ! Divergence of conserved variable fluxes (8 components)
      ...
      do lvl=0,this%amr%clvl()
         ...
         do while (mfi%next())
            ...
            do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
               ...
               else if (pVF(i,j,k,1).gt.VFhi) then; pQ(i,j,k,2)=0.0_WP; pQ(i,j,k,4)=0.0_WP; pQ(i,j,k,8)=0.0_WP
               end if
            end do; end do; end do
         end do
         call this%amr%mfiter_destroy(mfi)
   ...
   subroutine get_cfl(this,dt,cfl)
      ...
      do lvl=0,this%amr%clvl()
         ...
         get_viscmax: block
            ...
            type(amrex_mfiter) :: mfi
            type(amrex_box) :: bx
            real(WP), dimension(:,:,:,:), contiguous, pointer :: pQ,pVisc,pBeta,pCond,pDiff
            integer :: i,j,k,ierr
            real(WP) :: rho
            viscmax=0.0_WP
            call this%amr%mfiter_build(lvl,mfi)
