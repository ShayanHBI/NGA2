...
Usage:
  ./amrviz_cleanup.py amrviz/impact_relax_pTg --tmin 4.0 --tmax 9.0 --dt 0.1 --execute
...
def main():
    ...
    ap.add_argument("--dt", type=float, required=True, help="grid spacing to keep, e.g. 0.1")
    ap.add_argument("--tol", type=float, default=1e-3, help="tolerance for on-grid match")
    ...
    for idx, t in index_time.items():
        ...
        on_grid = abs(t - nearest) < args.tol
        if on_grid and args.tmin - args.tol <= t <= args.tmax + args.tol:
            keep.add(idx)
        else:
            drop.add(idx)

    tvals = index_time.values()
    print(f"{len(index_time)} output steps found (t={min(tvals):.4f} to {max(tvals):.4f})")
