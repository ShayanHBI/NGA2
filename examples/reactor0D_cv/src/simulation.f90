...
module simulation
   ...
   type(monitor) :: mfile,cflfile,consfile,igfile
   ...
   real(WP), dimension(:), allocatable :: YTI
...
contains
   ...
   !> Function that localizes y- boundary for SC
   ...
   subroutine simulation_init
      ...
      create_fc: block
         use multivdscalar_class, only: bcond,neumann,bquick
         ...
         ! call fc%add_bcond(name='xm',type=neumann,locator=xm_locator_sc,dir='-x')
         ! call fc%add_bcond(name='xp',type=neumann,locator=xp_locator   ,dir='+x')
         ! Configure implicit scalar solver
         ss=ddadi(cfg=cfg,name='Scalar',nst=13)
         ! Setup the solver
         call fc%setup(implicit_solver=ss)
         ! Allocate memory
         allocate (YTI(nspec+1)); YTI=0.0_WP
         ! Read in info
         call param_read('Pressure',fc%Pthermo)
         ! Initial composition and temperature
         do nsc=1,nspec+1
            if (param_exists('Initial '//trim(fc%SCname(nsc)))) then
               call param_read('Initial '//trim(fc%SCname(nsc)),YTI(nsc))
            end if
         end do
         YTI(1:nspec)=YTI(1:nspec)/sum(YTI(1:nspec))
         do nsc=1,nspec+1
            do k=fc%cfg%kmin_,fc%cfg%kmax_
               do j=fc%cfg%jmin_,fc%cfg%jmax_
                  do i=fc%cfg%imin_,fc%cfg%imax_
                     fc%SC(i,j,k,nsc)=YTI(nsc)
                  end do
               end do
            end do
            ! Sync
      ...
      create_ensight: block
         ...
         ens_out=ensight(cfg=cfg,name='reactor0D_cv')
      ...
      create_monitor: block
         ...
         call consfile%write()
         ! Create ignition monitor
         igfile=monitor(fs%cfg%amRoot,'ignition')
         call igfile%add_column(time%t,'Time')
         call igfile%add_column(fc%SC(1,1,1,nspec+1),'T')
         call igfile%add_column(fc%SC(1,1,1,sXC12H26),'XC12H26')
         call igfile%add_column(fc%SC(1,1,1,sHMN),'HMN')
   ...
   subroutine simulation_final
      ...
      deallocate (resU,resV,resW,Ui,Vi,Wi,resSC,resRHO,SCtmp,YTI)
