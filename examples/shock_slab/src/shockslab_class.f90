!> Definition of a shock-slab problem
module shockslab_class
   ...
   contains
      procedure :: initialize                      !< Initialize shock-slab simulation
      procedure :: step                            !< Advance shock-slab simulation by one time step
      procedure, private :: postproc               !< Postprocess shock-slab case
      procedure :: analyze_drops                   !< Output droplet analysis
      procedure :: output_monitor                  !< Monitoring for shock-slab case
      procedure :: output_ensight                  !< Ensight output for shock-slab case
      procedure, private :: prepare_viscosities    !< Prepare viscosities
      procedure :: apply_bconds                    !< Apply boundary conditions
      procedure :: finalize                        !< Finalize shock-slab simulation
   end type shockslab
...
contains
   ...
   subroutine postproc(this)
      ...
      implicit none
      class(shockslab), intent(inout) :: this
   ...
   !> Initialization of a shock-slab problem
   subroutine initialize(this,dx,meshsize,startloc,group,partition,continue_monitor)
      ...
      ! Check if this shockslab is a continuation, coming from remeshing
      ...
      create_config: block
         ...
         ! x is non-periodic; y and z are always periodic (1D problem)
         ...
         grid=sgrid(coord=cartesian,no=3,x=x,y=y,z=z,xper=xper,yper=yper,zper=zper,name='ShockSlab')
         ! Deallocate x/y/z
         deallocate(x,y,z)
         ! Create config
      ...
      initialize_timetracker: block
         this%time=timetracker(amRoot=this%cfg%amRoot,name='ShockSlab',print_info=.false.)
      ...
      create_ensight: block
         ...
         this%ens_out=ensight(cfg=this%cfg,name='ShockSlab',time_dependent_geometry=.true.)
      ...
      create_monitor: block
         ...
         this%mfile=monitor(this%fs%cfg%amRoot,'shockslab_sim',restart=monitor_continue)
         ...
         ! Create CFL monitor
         this%cflfile=monitor(this%fs%cfg%amRoot,'shockslab_cfl',restart=monitor_continue)
         ...
         this%consfile=monitor(this%fs%cfg%amRoot,'shockslab_cons',restart=monitor_continue)
         ...
         ! Create drop output
         this%dropfile=monitor(this%fs%cfg%amRoot,'shockslab_drop',restart=monitor_continue)
         ...
         ! Create mesh output
         this%meshfile=monitor(this%cfg%amRoot,'shockslab_mesh',restart=monitor_continue)
   ...
   !> Perform and output monitoring for the shockslab problem
   ...
   !> Output ensight files for the shockslab problem
   ...
   subroutine apply_bconds(this)
      ...
      if (.not.this%fs%cfg%xper.and.this%fs%cfg%iproc.eq.1) then
         do k=this%fs%cfg%kmino_,this%fs%cfg%kmaxo_; do j=this%fs%cfg%jmino_,this%fs%cfg%jmaxo_
            ...
            do i=this%fs%cfg%imino,this%fs%cfg%imin-1
               ! Copy primitive variables
               this%fs%RHOL(i,j,k)=this%fs%RHOL(this%fs%cfg%imin,j,k)
               this%fs%PL  (i,j,k)=this%fs%PL  (this%fs%cfg%imin,j,k)
               this%fs%IL  (i,j,k)=this%fs%IL  (this%fs%cfg%imin,j,k)
               this%fs%RHOG(i,j,k)=this%fs%RHOG(this%fs%cfg%imin,j,k)
               this%fs%PG  (i,j,k)=this%fs%PG  (this%fs%cfg%imin,j,k)
               this%fs%IG  (i,j,k)=this%fs%IG  (this%fs%cfg%imin,j,k)
               this%fs%U   (i,j,k)=this%fs%U   (this%fs%cfg%imin,j,k)
               this%fs%V   (i,j,k)=this%fs%V   (this%fs%cfg%imin,j,k)
               this%fs%W   (i,j,k)=this%fs%W   (this%fs%cfg%imin,j,k)
               this%fs%VF  (i,j,k)=this%fs%VF  (this%fs%cfg%imin,j,k)
